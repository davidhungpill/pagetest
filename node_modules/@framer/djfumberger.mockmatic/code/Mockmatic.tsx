...
function baseServerURL(url) {
    return "https://cors-anywhere.herokuapp.com/mockmatic.netlify.com/" + url
}
...
class Materials {
    ...
    updateTexture(screenImage: string) {
        ...
        // Fix for images not showing when viewing at http://127.0.0.1:4567
        // TODO: Find out from Framer team why this is broken by default ?
        screenImage = screenImage.replace("/preview/", "/")
...
export class Mockmatic extends React.Component<Props> {
    ...
    updateMaterials() {
        ...
        if (this.isFrameBasedImage()) {
            materials.glassFrontMaterial.opacity = Math.min(
                p.screenReflection,
                0.2
            )
...
class MockmaticDOMScreen extends React.Component<DOMScreenProps> {
    ...
    updateScreen() {
        ...
        this.objectPivot.position.set(
            (this.props.deviceX * 1.0) / this.sceneScale,
            (this.props.deviceY * 1.0) / this.sceneScale,
            (this.props.deviceZ * 1.0) / this.sceneScale
        )
        this.objectPivot.rotation.set(
            degrees_to_radians(this.props.deviceRotationX),
            degrees_to_radians(this.props.deviceRotationY),
            degrees_to_radians(this.props.deviceRotationZ)
        )
        this.objectPivot.scale.set(
            this.props.deviceScale,
            this.props.deviceScale,
            this.props.deviceScale
        )
    }
    ...
    render() {
        ...
        return (
            <div style={style} ref={this.container}>
                <div ref={this.childRef} style={childContainerStyle}>
                    {children}
                </div>
            </div>
        )
